...
describe("Basic Class Tests", function()
  ...
  it("Type Tests", function()
    ...
    local Class2 = class.class(
      {Class}, function(_) end, {z = types.String})
    local Class3 = class.class(
      {Class2}, function(_) end, {})
    local Class4 = class.class(
      {Class3}, function(_) end, {})
    local p2 = Class:new()
    local p3 = Class2:new()
    local p4 = Class4:new()
    ...
    assert.is_false(pcall(function() p2.z = "test" end))
    assert.is_false(pcall(function() p2.z = 0 end))
    assert.is_true(Class:is(p2))
    assert.is_true(class.Object:is(p2))

    assert.is_false(pcall(function() p3.x = "test" end))
    assert.is_true(pcall(function() p3.x = 0 end))
    assert.is_false(pcall(function() p3.y = "test" end))
    assert.is_true(pcall(function() p3.y = ChildClass:new(1, 2) end))
    assert.is_false(pcall(function() p3.z = 0 end))
    assert.is_true(pcall(function() p3.z = "test" end))
    assert.is_true(Class:is(p3))
    assert.is_true(class.Object:is(p3))

    assert.is_false(pcall(function() p4.x = "test" end))
    assert.is_true(pcall(function() p4.x = 0 end))
    assert.is_false(pcall(function() p4.y = "test" end))
    assert.is_true(pcall(function() p4.y = ChildClass:new(1, 2) end))
    assert.is_false(pcall(function() p4.z = 0 end))
    assert.is_true(pcall(function() p4.z = "test" end))
    assert.is_true(Class:is(p4))
    assert.is_true(class.Object:is(p4))
  end)
end)
